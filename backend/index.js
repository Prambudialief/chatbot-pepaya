...
// ─────────────────────────────────────────────
// GROQ — generate jawaban (GRATIS)
// API kompatibel dengan OpenAI format
// Model: llama-3.3-70b-versatile (paling cerdas di Groq)
// ─────────────────────────────────────────────
async function generateWithGroq(pertanyaan, konteks) {
      ...
      messages: [
        {
          role: "system",
          content:
            "Kamu adalah asisten ahli pertanian yang membantu mendiagnosis dan memberikan informasi tentang penyakit tanaman pepaya. " +
  ...
  if (!response.ok) {
    ...
    if (response.status === 429) {
      throw new Error("Groq rate limit. Coba lagi dalam beberapa detik.");
    }
...
app.post("/tanya", async (req, res) => {
  ...
  try {
    ...
    if (chunkRelevan.length === 0) {
      jawaban =
        "Maaf, saya tidak menemukan informasi yang relevan dengan pertanyaan Anda dalam basis pengetahuan penyakit pepaya.";
    } else {
      // 3. Susun konteks
      ...
      // 4. Generate dengan GROQ ← (bukan Gemini lagi)
      console.log("   🤖 Mengirim ke Groq...");
      jawaban = await generateWithGroq(pertanyaan, konteksTeks);
...
app.get("/history/:userId", async (req, res) => {
  ...
  try {
    ...
    res.json({ userId, total: history.length, data: history });
  } catch (err) {
...
initVectorStore().then(() => {
  app.listen(PORT, () => {
    ...
    console.log(`   DELETE /history/:userId   → hapus riwayat`);
    console.log("─".repeat(55));
    console.log(`   🤖 LLM : Groq llama-3.3-70b-versatile (GRATIS)`);
    console.log(`   🗄️  DB  : FAISS local + Firestore`);
    console.log("=".repeat(55));
  });
});
